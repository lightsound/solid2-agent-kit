...
- `npm run check:upstream -- --versions baseline` must pass for a new probe; `npm run check:upstream` runs every probe on the newest dist-tagged prereleases. `.github/workflows/upstream-watch.yml` runs it weekly and keeps one `upstream-watch` issue open while a claim changed or a watched package moved past `baseline` in `bin/baseline.mjs`.
- `baseline` is also `doctor`'s floor, with no separate "minimum compatible" version: it is the release the guidance is written against (and the probes verify), so an older one is unsupported and fails `doctor` (and the Claude Code Stop gate). Raising a `baseline` or adding a package to the table therefore requires projects to upgrade once they update the kit; state it in the release PR.
