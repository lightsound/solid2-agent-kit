...
- `baseline` is also `doctor`'s floor, with no separate "minimum compatible" version: the probes verify only `baseline`, so an older release is unverified and fails `doctor` (and the Claude Code Stop gate). Raising a `baseline` therefore requires projects to upgrade that package once they update the kit; state it in the release PR.
